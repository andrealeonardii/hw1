<?php
    ...
    // Controlla se  esiste
    if ($result->num_rows > 0) {
      $query = "DELETE FROM Aggiungiviaggio WHERE user_id = $idUtente AND viaggio_id = $viaggio_id;";

      if (mysqli_query($conn, $query)) {
          mysqli_close($conn);
          $errore ="Hai eliminato il seguente viaggio:";
      }else {
        $errore = 'Errore nel caricamento del DB';
      }
    }else {
          $errore = 'Non puoi eliminare un viaggio che non hai prenotato';
